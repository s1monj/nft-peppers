...
#awk -F '\t' '{print $3}' gff3/GCA_000512255.2/genomic.gff

#awk -F '\t' '$3=="gene"' gff3/GCA_000512255.2/genomic.gff
...
# prune for exons and start coordinates only
# for g in "${genome_ids[@]}"
# do
...
# cat test.txt | jq --raw-input --slurp 'split("\n") | map(split(" ")) | .[0:-1] | map( { "src": .[0], "dest": .[1] } )'
